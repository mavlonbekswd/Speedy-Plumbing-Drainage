...
import { ANSWERED_LINE, BOOKED_WORK_LINE, PRICE_PROCESS_LINE, SAME_DAY_LINE } from "@/lib/claims";
...
//
// Order, urgent and booked alike: eyebrow, H1, the buttons, then the three one-line facts, then
...
export default function ServiceHero({
  ...
  // is the person who comes.") is answer card 4 on every page, and at 390x844 the first screen is
  // a decision, not a page: hero copy is budgeted at 60 words (tests/town-headline.spec.ts).
  const answeredLead = `${ANSWERED_LINE.split(". ")[0]}.`;
  const facts = [PRICE_PROCESS_LINE, urgent ? SAME_DAY_LINE : BOOKED_WORK_LINE, answeredLead];
